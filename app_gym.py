...
def cargar_datos():
    try:
        ...
        # --- CORRECCIÓN DE FORMATO DE FECHA A DD-MM-YYYY ---
        for col in ["fecha_ingreso", "fecha_vencimiento"]:
            if col in df.columns:
                df[col] = df[col].astype(str).str.replace(r'[TZ].*', '', regex=True).str.strip()
                fechas_dt = pd.to_datetime(df[col], errors='coerce', format='mixed', dayfirst=True)
                df[col] = fechas_dt.dt.strftime('%d-%m-%Y').fillna(df[col])
...
if not df_clientes.empty and "valor_pagado" in df_clientes.columns:
    try:
        valores = pd.to_numeric(df_clientes["valor_pagado"], errors="coerce").fillna(0)
    ...
    if not df_clientes.empty:
        ...
        try:
            ...
            st.download_button(
                ...
                file_name=f"clientes_gym_{datetime.today().strftime('%Y-%m-%d')}.xlsx",
